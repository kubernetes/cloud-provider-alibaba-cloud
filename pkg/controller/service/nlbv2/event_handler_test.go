...
func TestEventHandlerNoopBranches(t *testing.T) {
	...
	epHandler := NewEnqueueRequestForEndpointEvent(kubeClient, record.NewFakeRecorder(100))
	nodeHandler := NewEnqueueRequestForNodeEvent(kubeClient, record.NewFakeRecorder(100))
	esHandler := NewEnqueueRequestForEndpointSliceEvent(kubeClient, record.NewFakeRecorder(100))
