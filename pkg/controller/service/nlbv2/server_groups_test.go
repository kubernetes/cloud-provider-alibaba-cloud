...
func TestSetWeightBackends(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := setWeightBackends(tt.mode, tt.backends, tt.weight, pointer.Int(DefaultServerWeight))
			tt.validate(t, result)
		})
	}
}
...
func TestAddressTypeToIPVersionType(t *testing.T) {
	assert.Equal(t, model.IPv4, addressTypeToIPVersionType(discovery.AddressTypeIPv4))
	assert.Equal(t, model.IPv6, addressTypeToIPVersionType(discovery.AddressTypeIPv6))
	assert.Equal(t, model.IPv4, addressTypeToIPVersionType(discovery.AddressTypeFQDN))
}
