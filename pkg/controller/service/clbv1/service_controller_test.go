...
func TestReconcileService_ReconcileLoadBalancerResources(t *testing.T) {
	recon := getReconcileService()
	svc := getDefaultService()
	reqCtx := getReqCtx(svc)

	err := recon.reconcileLoadBalancerResources(reqCtx)
	assert.NoError(t, err)
}

func TestReconcileService_UpdateServiceStatus_LBNil(t *testing.T) {
	recon := getReconcileService()
	svc := getDefaultService()
	reqCtx := getReqCtx(svc)
...
func TestHasInvalidBackendInVServerGroups(t *testing.T) {
	assert.False(t, hasInvalidBackendInVServerGroups(nil))
	assert.False(t, hasInvalidBackendInVServerGroups([]model.VServerGroup{
		{InvalidBackends: nil},
	}))
	assert.True(t, hasInvalidBackendInVServerGroups([]model.VServerGroup{
		{InvalidBackends: []model.BackendAttribute{{ServerId: "i-1"}}},
	}))
}
