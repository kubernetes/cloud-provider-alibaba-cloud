...
func TestPodNumberAlgorithm(t *testing.T) {
	t.Run("eni mode", func(t *testing.T) {
		...
		result := podNumberAlgorithm(helper.ENITrafficPolicy, backends, DefaultServerWeight)
	...
	t.Run("cluster mode", func(t *testing.T) {
		...
		result := podNumberAlgorithm(helper.ClusterTrafficPolicy, backends, DefaultServerWeight)
		assert.Equal(t, DefaultServerWeight, result[0].Weight)
		assert.Equal(t, DefaultServerWeight, result[1].Weight)
	})
	...
	t.Run("local mode", func(t *testing.T) {
		...
		result := podNumberAlgorithm(helper.LocalTrafficPolicy, backends, DefaultServerWeight)
