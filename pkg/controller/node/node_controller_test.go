...
func TestBatchWorker(t *testing.T) {
	...
	t.Run("batch worker process nodes", func(t *testing.T) {
		...
		go func() {
			defer wg.Done()
			recon.batchWorker(ctx, 0)
		}()
		// cancel runs first (LIFO), then wg.Wait blocks until the worker
		// goroutine fully exits, so it no longer reads the global config
		// while the outer defer restores it.
		defer wg.Wait()
		defer cancel()
