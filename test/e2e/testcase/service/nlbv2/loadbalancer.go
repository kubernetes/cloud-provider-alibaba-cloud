	...
	ginkgo.Describe("nlb service controller: loadbalancer", func() {
		...
		ginkgo.Context("can not find nlb", func() {
			...
			ginkgo.It("auto-created nlb & delete tag", func() {
				...
				gomega.Expect(err).To(gomega.BeNil())
				key := helper.TAGKEY
				err = f.Client.CloudClient.UntagNLBResources(context.TODO(),
					nlb.LoadBalancerAttribute.LoadBalancerId, []*string{&key})
				gomega.Expect(err).To(gomega.BeNil())
			...
			ginkgo.It("auto-created nlb & delete tag & change name", func() {
				...
				key := helper.TAGKEY
				err = f.Client.CloudClient.UntagNLBResources(context.TODO(),
					slb.LoadBalancerAttribute.LoadBalancerId, []*string{&key})
				gomega.Expect(err).To(gomega.BeNil())
